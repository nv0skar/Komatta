    ...
    pub fn decode(&self) -> Result<Vec<u8>, String> {
        if let Ok(decoded) = {
            if self.hex {
                ...
                if let Ok(base64) = base64::decode(self.value.clone()) {
                    Ok(base64)
                } else {
                    Err(())
                }
...
fn main() {
    let parsed = Command::new(env!("CARGO_PKG_NAME"))
        ...
        .args([
            ...
            Arg::new("hex").help("encrypted data and keys will be encoded in Base16")
                .long("hex")
                .required(false)
        ...
        .subcommands([
            Command::new("encrypt").args([
                ...
                Arg::new("keySize").help(format!("if the key is missing, a randomly generated byte stream with the specified size will be generated (accepts {})", CRYPT_KEY_SIZE).as_str())
                    .long("keySize")
                    .conflicts_with("key")
                    .takes_value(true)
                    .required(false)
                    .value_parser(clap::value_parser!(u8)),
                Arg::new("noSigning").help("will disable signing")
                    .long("noSigning")
                    .required(false)
                    .action(ArgAction::SetTrue),
                Arg::new("sizes").help("set the sizes for the crypt")
                    .long("sizes")
                    .conflicts_with("key")
                    .takes_value(true)
                    .required(false)
